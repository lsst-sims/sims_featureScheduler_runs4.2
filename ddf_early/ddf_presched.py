...
def ddf_slopes(ddf_name, raw_obs, night_season, season_seq=30, min_season_length=0 / 365.25,
    ...
    elif ddf_name == "EDFS_b":
        # EDFS_b ddf visits are allocated some other way
        season_vals = season_vals * 0

    else:
        # Here's where we kludge in some logic to front-load
        full_seasons = np.where(np.round(season_vals) == np.round(season_vals).max())[0]

        tot = np.sum(season_vals[full_seasons])

        early = tot * boost_factor
        later = tot * (1. - boost_factor)

        n_early = early/n_years
        n_later = later/(full_seasons.size - n_years)

        season_vals[full_seasons[0:n_years]] = n_early
        season_vals[full_seasons[n_years:]] = n_later
